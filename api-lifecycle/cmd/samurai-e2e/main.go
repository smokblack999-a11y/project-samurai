...
func main() {
	...
	flag.Parse()
	if *openapi == "" || *logs == "" || *endpoint == "" {
		panic("usage: samurai-e2e -openapi FILE -logs FILE -endpoint PATH [-method GET] [-out FILE] [-bundle-out FILE]")
	}

	f, err := os.Open(*openapi)
	if err != nil { panic(err) }
	defer f.Close()
	endpoints, err := spec.Read(f)
	if err != nil { panic(err) }
	var epFound bool
	var ep = endpoints[0]
	...
	if err != nil { panic(err) }
	defer lf.Close()
	records, err := consumer.ReadJSONL(lf)
	if err != nil { panic(err) }
	...
	evidence := audit.Run(audit.Input{
		...
		ReplacementHealthy: ep.ReplacementHealthy,
		MigrationCompletion: ep.MigrationCompletion,
	...
	if *bundleOut != "" {
		bundle, err := remediation.Build(evidence)
		if err != nil { panic(err) }

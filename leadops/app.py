...
app = FastAPI(title="Telegram LeadOps AI", version="0.1.1")
...
def baseline(message: Message) -> Decision:
    ...
    needs: list[str] = []
    if any(x in t for x in ("цена", "стоимость", "price", "cost")):
        needs.append("price")
    if any(x in t for x in ("есть", "свободно", "доступно", "можно", "available")):
